...
int *copyArray2D_V1(size_t nrows, size_t ncols, int array2D[][ncols]) {
    int *copy = (int *)malloc(sizeof(int) * nrows * ncols);
    int *copyp = copy;
 ...
 * @param ncols the number of columns in the input array
 * @param array2D the input 2D array
 * @return the allocated array of array pointers
 */
int **copyArray2D_V2(size_t nrows, size_t ncols, int array2D[][ncols]) {
    // allocate memory for copied pointer array
    int **copy = (int **)malloc(sizeof(int *) * nrows);
    // allocate memory for every pointer in copied pointer array
    for (int i = 0; i < nrows; i++) {
        copy[i] = (int *)malloc(sizeof(int) * ncols);
    }
    // assign int number for every element in the copied array
    for (int i = 0; i < nrows; i++) {
        for (int j = 0; j < ncols; j++) {
