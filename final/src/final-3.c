...
Card *createCard(Value value, Suit suit, Card *card) {
	// your code here
    if (card == NULL) card = (Card *)malloc(sizeof(Card));
...
Deck *createDeck(Deck *deck) {
	// your code here
    if (deck == NULL) deck = (Deck *)malloc(sizeof(Deck));

    deck->ncards = 0;

    for (Suit s = Hearts; s <= Spades; s++)
        for (Value v = Ace; v <= King; v++)
            deck->cards[deck->ncards++] = *createCard(v, s, NULL);

	return deck; // TEMPORARY
}
...
char *cardToString(Card *card, char *str) {
	// your code here
    if (str == NULL) str = (char *)malloc(sizeof(char) * 20);
    sprintf(str, "%s of %s", valueName[card->value], suitName[card->suit]);
